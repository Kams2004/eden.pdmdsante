...
const CommissionView: React.FC<CommissionViewProps> = ({ selectedPatients = [] }) => {
  ...
  useEffect(() => {
    const filterPatientsByDateAndSearch = () => {
      ...
      setFilteredPrescriptionPatients(filterByDateAndSearch(prescriptionPatients));
      setFilteredRealisationPatients(filterByDateAndSearch(realisationPatients));
      setCurrentPage(1);
    };
  ...
  return (
    <div className="w-full space-y-6">
      ...
      (du 21 Mars au {new Date().toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })})
    </p>
        ...
        <div className="px-6 py-4 bg-white border-b border-slate-200">
          ...
          {searchTerm && (
            <p className="text-sm text-blue-600 mt-2">
              Filtrage actif pour: "{searchTerm}" - {currentPatients.length} résultat(s) trouvé(s)
            </p>
          )}
        </div>
            ...
            {showDatePicker && (
              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-white rounded-lg border border-slate-200 shadow-sm mb-4">
                <div className="flex flex-col">
                  <label className="text-sm font-bold text-gray-800 mb-1">Date de Début</label>
                  <DatePicker
                    selected={startDate}
                ...
                <div className="flex flex-col">
                  <label className="text-sm font-bold text-gray-800 mb-1">Date de Fin</label>
                  <DatePicker
                    selected={endDate}
            ...
            <table className="w-full min-w-[800px]">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Patients</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Examens</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Commissions</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Dates</th>
                  <th className="px-4 py-3 text-left text-sm font-semibold text-gray-600">Statuts</th>
                </tr>
              </thead>
              <tbody>
                {paginatedPatients.map((patient) => (
                  <tr key={patient.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 font-medium">{patient.id}</td>
                    <td className="px-4 py-3 text-sm text-gray-800 font-medium">{patient.name}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{patient.examination}</td>
                    <td className="px-4 py-3 text-sm text-gray-600 font-semibold">{patient.commission}</td>
                    <td className="px-4 py-3 text-sm text-gray-600">{patient.transferDate}</td>
                    <td className="px-4 py-3">
                      <div className={`border-[0.4px] border-gray-300 rounded-[10px] p-[5px] text-center text-white ${getFactureColor(patient.facture)}`}>
